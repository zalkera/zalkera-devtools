...
const WIRES = [
    // ── 판을 읽는 일과 **판정하는 일**은 갈라 두었다 ─────────────────────────────
    // 🔴 판정(`versionFrom`)에는 시험이 섰지만 **배선은 아무도 안 물었다**(설계자 심의 실측):
    //    `version()` 이 `versionFrom` 을 안 부르고 `"0.0.0"` 폴백을 재내장해도 전량 초록이었다.
    //    그 값은 어떤 최소판에도 못 미쳐 서버가 `UPGRADE_REQUIRED` 를 내는데, **업데이트해도 안
    //    고쳐진다** — 낡은 것은 판이 아니라 설치이기 때문이다. 가른 이유가 그 교착을 막는 것이니,
    //    가른 채로 안 부르면 가르지 않은 것과 같다.
    [
        "packages/cli/src/context.ts",
        "return versionFrom(raw);",
        "**판정을 갈라 놓고 안 부르면 가르지 않은 것과 같다.** 폴백을 재내장하면 설치 파손이 " +
            "「낡은 판」으로 둔갑해, 업데이트해도 안 고쳐지는 교착이 돌아온다",
    ],
    [
        "packages/cli/src/context.ts",
        "loginHow: LOGIN_HOW",
    ...
    //    `ELECTRON_RUN_AS_NODE=1`(＝`runtime.env`)이 함께 가야 Node 로 돈다 — 안 주면 미리보기가
    //    Node 대신 **VS Code 새 창**을 띄운다(같은 레포 `runtime.ts` KDoc 의 실측).
    //
    // ⚠ 종전 앵커는 `.mcp.json` 에 적히던 자리(`env: runtime.env,`)였는데 그 문을 걷었다.
    //   **지우지 않고 살아 있는 자리로 옮긴다** — 같은 함정이 미리보기 경로에 그대로 있고,
    //   거기는 앵커가 없었다.
    [
        "packages/vscode/src/extension.ts",
        "          extraEnv: runtime.env,",
        "**동봉 Node 를 환경 없이 부르면 Node 가 아니라 VS Code 새 창이 뜬다.** 미리보기 서버가 " +
            "그 프로세스이고, 안 뜨는 이유가 화면에 안 나온다 — 사람은 「미리보기가 안 된다」만 본다",
    ],
    ...
    //    줄로 부수효과를 조건부로 만들어도 조각은 그대로 있다. 들여쓰기가 그 길을 좁힌다.
    //
    // ⚠ **막지는 못한다.** 앞줄에 **중괄호 없는 if** 를 세우면 들여쓰기가 그대로라 조각이 살아남고
    //    전량 verify 가 초록이다(설계자 심의 실측). 이 검사기는 부분문자열이라 문법을 안 본다 —
    //    의도적 우회를 막는 물건이 아니라 **무심코 옮기는 것**을 막는 물건이다. 같은 축의 알려진
    //    우회는 `doc/DEFERRED.md` 에 모은다.
    //
    ...
    [
        // ⚠ **이 줄을 지워도 시험은 전건 초록이다.** `displayPath` 의 홈 갈래 시험 셋이 계속
        //    통과하는 채로 `~` 접기만 죽는다 — 「시험이 배송 안 되는 동작을 보증한다」는, 이 축이
        //    스스로 지적하고 고친 바로 그 형상이다(Fable 기능 축이 되짚었다).
        "packages/vscode/src/extension.ts",
        "home: homedir(),",
        "경로가 `~` 로 안 접혀 묶음 머리에서 머리가 잘린다 — 시험은 초록인 채로 조용히 죽는다",
    ],
    [
    ...
    [
        ...
        // ⚠ **두 자리다. 둘 다 같은 이유로 캡처를 들고 간다** — 「제안 흐름의 받기가 라이브 사이트를
        //    읽으면 엉뚱한 사이트의 소스를 받는다」. ⑴ `fetch` 갈래(새 빈 폴더로 받기) ⑵ `open` 갈래에서
        //    폴더가 사라졌을 때의 「새로 받기」. ⑵ 를 종전처럼 사이드바 「소스 다운로드」로 안내하면
        //    그 버튼은 **무캡처 배선**이라 이 창의 유효 사이트를 받는다 — 방금 고른 사이트가 아니다.
        //    한 자리로 줄이면 그 오안내가 되살아난다.
