...
- `parseSourceMark`: format 1·2 를 받고 그 밖은 null. `holdsSameRevision`: `linked` 는 항상 false
  (판 칸이 없다). `published` 도 제안에서 뺀다 — 그 판은 **올린 것**의 판이지 그 폴더가 지금 그 판의 사본이라는 뜻이 아니다(심의 확정) — 초판은
...
판정 순서는 **소속이 폴더 유무보다 앞**이다. 폴더 유무를 먼저 보면, 소스 아닌 폴더에 링크만
남은 창에서 전역에 적히는데 병합 조회는 그 링크가 이겨 화면과 실동작이 갈린다.

`none` 을 소비하는 자리는 확장의 `saveTenant` 다. 삼항으로 쓰면 `none` 이 전역으로 흘러가므로
전수 `switch` 로 두고 `never` 로 칸 누락을 타입이 잡게 한다 — 배선 검사도 그 두 줄을 고정한다.
...
5. **닫힘** — 발행의 표식 결정화(§4.6). 「후보」로 적어 둔 안이 실제로 섰다: 소속 없는 폴더의
   발행 확인 모달이 **소속이 정해진다는 사실을 먼저 말하고 단추도 다르다**(「이 사이트로 올리고
   연결」). 그 갈래를 만든 것은 다른 축이지만(소속 모르는 폴더의 밀림), 이 쟁점이 요구한 고지가
   그 안에 들어 있다. 판정: 유지 + 고지.
...
- **보안**: `decideTenantScope` 의 `none` 을 소비하는 줄이 어떤 그물에도 안 걸렸다 — 삼항이
...
권고 반영: 갱신 타이머의 게이트 우회 차단 · 표식 읽기의 심링크·크기 상한 · 표식 사이트 코드의
모양 검증 · 같은 사이트 재연결 시 받기 표식 보존 · 판정 순서(소속 우선) · 사이드바 항목을
누를 수 있게 · 배송 문서의 절대문·과약속 정정.
