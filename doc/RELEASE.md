...
| 판 | 커밋 | 받은 심의 | Fable |
|---|---|---|---|
| **0.23.1** | (이 판) | 0.23.0 반려 조건 이행 · **Opus 3축** | **미이수** — 조건 셋을 이행했다: ⑴ `RELEASE.md` 의 전제를 사실로(걷은 것은 **경로 하나**이고 같은 권한은 원격 MCP 에 그대로 있다 — 양쪽 도구 목록을 실측으로 실었다) ⑵ 그 위에 선 문면 정정(`MANUAL.md` 는 원래 정확했다 — 발행·되돌리기가 열린다고 적는다) ⑶ `isOurEntry` 그물 — 느슨해지는 변이 **셋**(`runsOurVerb` 제거 · http `oauth` 짝 제거 · 짝 하나만 보기)이 종전에 전부 초록이었고 지금은 전부 red 다. 조이는 쪽 통제군(전부 거절)도 red 4. 재현: 각 변이 후 `npm run test -w @zalkera/devtools-core`. 2회전 대기 |
