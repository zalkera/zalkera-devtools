...
| 판 | 커밋 | 받은 심의 | Fable |
|---|---|---|---|
| **0.23.0** | (이 판) | 「소스 다루게 하기」 철거 · 설계 없음(오너 판단) · **Opus 구현** + **Opus 3축** 2회전 | **미이수** — 1회전이 **반려**했다: ⑴ 전제가 거짓(걷은 문이 아니라 **남긴 문**에 `publish`·`activate_revision` 이 실려 있다) ⑵ 새로 쓴 배송 문면 셋이 그 거짓 위에 섰다 ⑶ 지운 시험이 물던 `isOurEntry` stdio 갈래가 무그물이 됐다(느슨해지는 변이 2종 생존). 이행 후 2회전 대기 |
