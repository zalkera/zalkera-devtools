...
| **0.20.2** | `c96f78b` | CLI 를 npm `@zalkera/cli` 로 내는 길 · 설계 없음 · **Opus 3축 2회전** | **필요** — Opus 단독. ⚠ 한때 「0.21.0 심의가 함께 봤다」로 올렸다가 되돌렸다: 그 심의가 본 것은 **vsix 에 동봉해 절대경로로 부르는 형상**이고, 이 판은 **npm `@zalkera/cli` 로 내는 길**이다 — 겹치지만 같지 않다. 발행 경로·패키지 경계·스코프 선택은 그 심의의 대상이 아니었다 |
| **0.20.1** | `9a78ccb` | 확장 번들 동기화(핸드셰이크 안내 · 오류 코드) · 고객 가시 변경 0 · **Opus 검토** | **필요** — Opus 단독 |
