...
| **0.17.0** | `7537172` | 발행 취소 + 사이드바 줄 문면 · 설계 심의 → **Opus 3축** → **Fable 조건 이행 확인** | **부분** — 3축을 본 것은 Opus 다. Fable 은 조건 이행 확인 회전에서 F2 를 「실질 명목」으로 반려했고 그것을 고쳤다(등가 변이 3종 red 확인). 남은 등가 변이는 `DEFERRED.md` 에 있다 |

✅ **0.16.0 발행 완료(2026-08-26).** 아래 두 선행 조건은 **충족된 뒤** 발행했다 — `zalkera-spring`
`6e3d56fe` 상용 반영(선언 대조 + 재시도 가능 거절의 zip 보존 + `limit`), 관측 상행 정상(11/11·failStreak 0).
다음 판을 낼 때 같은 함정을 다시 밟지 않도록 조건을 **그대로 남긴다**:

⚠ **백엔드 선행 배포가 조건이다 — 조건이 둘이고, 둘째를 한 번 놓쳤다.**
...
| 0.15.0 | (0.16.0 에 흡수) | 백엔드 명세 A 하행 · Opus 변이 검증(판정 3종·배선 2종) · **3축 미이수** | 진행 |
