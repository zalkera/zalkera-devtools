...
|---|---|---|
| **1순위 · 0.20.0 (memo184 T3)** | T3 2회전(Fable 3축) 지적 **반영분**의 회귀 확인. 3회전은 **Opus** 가 봤고 조건 6건 중 ⑥에서 🟠 하나(빌드 창 갈래가 방향을 안 봄)를 잡아 이행했다 | 2회전을 낸 것이 Fable 이다. **자기가 낸 조건이 이행됐는지는 낸 쪽이 봐야 한다** — T1·T2 때 매 2회전이 직전 반영의 회귀를 잡았다 |
...
| 판 | 커밋 | 받은 심의 | Fable |
|---|---|---|---|
| **0.20.0** | `<이 판의 커밋>` | memo184 T3(CLI 발행·되돌리기·버리기) + 사이트 전환 경로 표시 · 설계(**Fable**, memo184) + **Opus 구현** + **Fable 3축**(T3 1·2회전) + **Opus 조건 이행 확인**(3회전) | **부분** — 설계와 3축은 Fable 이 봤다. 다만 2회전 지적을 반영한 **그 반영분**을 본 것은 Opus 다(Fable 한도). 재개 지점은 아래 🔖 |
