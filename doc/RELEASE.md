...
⚠ **0.16.0 은 백엔드 선행 배포가 조건이다.** HTTP 역직렬화가 미지 필드를 관용하므로, 필드를
모르는 서버는 `baseRevisionNo` 를 400 이 아니라 **조용히 무시**한다. 그러면 표식 있는 폴더는
고지도 안 뜨고 방어도 없는 상태가 되어 **「조용히 무보호」가 전 사용자에게 재현된다** — 이
트랜치가 없애려던 그것이다. 발주서 §B 의 「배포 순서: 백엔드 먼저」가 그 이유다.
