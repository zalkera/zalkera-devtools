...
| **1순위 · 0.20.0** | ⑴ T3 2회전(Fable 3축) 지적 **반영분**의 회귀 확인 — 3회전은 **Opus** 가 봤고 조건 6건 중 ⑥에서 🟠 하나(빌드 창 갈래가 방향을 안 봄)를 잡아 이행했다 · ⑵ **사이트 전환 경로 표시**(설계 없이 들어온 변경) | 2회전을 낸 것이 Fable 이다. **자기가 낸 조건이 이행됐는지는 낸 쪽이 봐야 한다** — T1·T2 때 매 2회전이 직전 반영의 회귀를 잡았다 |
