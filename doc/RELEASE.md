...
| 0.16.0 | (이 판) | 동시 업로드 최소 방어(§B) · 설계 Fable · **3축 미이수** | 진행 |

⚠ **0.16.0 은 백엔드 선행 배포가 조건이다 — 조건이 둘이고, 둘째가 지금 상용에서 참이 아니다.**

⑴ **필드를 아는 서버여야 한다.** HTTP 역직렬화가 미지 필드를 관용하므로 모르는 서버는
   `baseRevisionNo` 를 400 이 아니라 **조용히 무시**한다. 그러면 표식 있는 폴더도 고지 없이 무방어가
   되어 **「조용히 무보호」가 전 사용자에게 재현된다** — 이 트랜치가 없애려던 그것이다.

⑵ **`zalkera-spring` 의 `d93536be`(재시도 가능한 거절은 zip 을 보존) 가 상용에 있어야 한다.**
   ⚠ 설계 정본(`doc/DESIGN-concurrent-upload.md`)은 「§4.1 서버 구현됨·상용 배포됨」이라 말하는데,
   그것은 **선언 대조**만 가리킨다. 지금 상용은 confirm 이 **성패 무관** 아카이브를 지우고,
   `UPLOAD_BASE_MOVED` 는 그 `try` 안에서 난다 — 즉 사람이 「그대로 올리기」를 눌러도 서버가
   **없는 키**를 찾아 **매 시도 실패**한다. 탈출구가 죽은 채로 방어만 켜지는 것이 최악이다.

**배포 순서: 백엔드 두 커밋(`d93536be`·`1b163da9`) main 머지 + JAR 상용 반영 → 0.16.0 발행.**
역순은 구 확장에 무해하지만(필드·`limit` 을 안 보낸다) 그 반대는 아니다. `limit` 은 순서 무관이다.


| 0.15.0 | (이 판) | 백엔드 명세 A 하행 · Opus 변이 검증(판정 3종·배선 2종) · **3축 미이수** | 진행 |
| 0.14.1 | `ed7bceb` | 보안 축 지적 + T5b 심의 조건 이행 · **3축 미이수** | 진행 |
| **미발행** — 0.14.1 이후 | `56e4343` | 없음 — `npm run verify` 만 지났다(출력 패널 로그 2줄) | **필요** |
...
| 0.13.0 | `009a1ad`·`29f9d99`·`9fb2e01` + 성능 반영 | 설계(Fable 2판) + Opus 3축 | **판으로는 미이수** — 고객 급건으로 Fable 앞서 발행. 그 트랜치의 Fable 3축은 뒤이어 돌았고 조건은 0.13.1 에 실렸다 |
