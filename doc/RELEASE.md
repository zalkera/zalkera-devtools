...
|---|---|---|
| **1순위 · 0.21.0** | T4 전체 — 로컬 MCP 서버(SDK 없이 stdio 직접 구현 · 도구 이름 접두) · 「AI 연결」 묶음 분리 · CLI 를 vsix 에 **동봉**해 절대경로로 부르는 형상. **설계면이 셋 새로 열렸다**(설계에 없던 판단) | 새 계약 셋을 구현자 자기 검수로 닫았다 |
| **2순위 · 0.20.0** | ⑴ T3 2회전(Fable 3축) 지적 **반영분**의 회귀 확인 — 3회전은 **Opus** 가 봤고 조건 6건 중 ⑥에서 🟠 하나(빌드 창 갈래가 방향을 안 봄)를 잡아 이행했다 · ⑵ **사이트 전환 경로 표시**(설계 없이 들어온 변경) | 2회전을 낸 것이 Fable 이다. **자기가 낸 조건이 이행됐는지는 낸 쪽이 봐야 한다** — T1·T2 때 매 2회전이 직전 반영의 회귀를 잡았다 |
| **3순위 · memo184 T3 설계면** | 버리기 동의를 **두 자산군으로 가르는** 형상(서버가 `errors[].field` 로 짚고 CLI 가 그것으로 분기) · 되돌리기 착지 판을 응답에서 읽는 형상 | 계약 모양을 새로 정한 자리다. 설계 판단이라 구현자 자기 검수로 닫으면 안 된다 |
| **2순위 · 이름 방어 스텁** | `zalkera-devtools`·`zalkera-cli` 0.0.1 — **전용 3축 없이 npm 에 나갔다.** T4 3회전이 곁다리로 「발행 명령을 안 부르고 굽고 멈춘다」만 확인했다. 실행 표면이 0(파일 셋 · `bin` 없음 · 설치해도 `.bin` 에 안 생김 — 실측)이라 대가는 작아 보이지만, **규칙(main 푸시 전 3축)을 건너뛴 것은 사실이다** | 되돌릴 수 없다(npm 72시간). 다음에 같은 자리를 낼 때의 판단 근거로 |
| 4순위 · 판 표 소급 | 이 표에서 **필요** 로 남은 줄(`56e4343`) | 오래 밀린 것이라 급하지 않다 |
...
| 판 | 커밋 | 받은 심의 | Fable |
|---|---|---|---|
| **0.21.0** | `6eb7db4` | memo184 T4(로컬 MCP 4도구 · `preview`) + 「AI 연결」 묶음 · 설계(**Fable**, memo184 §2.6) + **Opus 구현** + **Opus 4회전** | **필요** — Opus 단독. 매 회전이 🔴 를 냈고 **둘은 직전 회전 반영이 넣은 회귀**였다(스칼라 JSON 이 서버를 죽임 · 동봉 Node 가 Electron 으로 뜸). 4회전에서 새 결함 0 |
| **0.20.2** | `c96f78b` | CLI 를 npm `@zalkera/cli` 로 내는 길 · 설계 없음 · **Opus 3축 2회전** | **필요** — Opus 단독 |
| **0.20.1** | `9a78ccb` | 확장 번들 동기화(핸드셰이크 안내 · 오류 코드) · 고객 가시 변경 0 · **Opus 검토** | **필요** — Opus 단독 |
| **0.20.0** | `d4dec76` | ⑴ memo184 T3(CLI 발행·되돌리기·버리기) — 설계(**Fable**, memo184) + **Opus 구현** + **Fable 3축**(1·2회전) + **Opus 조건 이행 확인**(3회전) · ⑵ 사이트 전환 경로 표시 — **Opus 단독**(설계 없음 · Opus 3축) | **부분** — Fable 이 본 것은 T3 의 설계와 1·2회전까지다. **2회전 반영분과 ⑵ 는 Opus 단독.** 재개 지점은 위 🔖 |
