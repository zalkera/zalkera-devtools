...
`finally` 뒤로 3~5초 동안 우리 사건이 밀려오고, 그 뒤 심은 값이 버려져 ×2 훑기가 돌아왔다(1회전
성능 실측). 실측 표(1회전 성능): 사건당 거름 비용 1.7µs · Next 16 Turbopack 콜드 첫 페이지 4,101사건/s
전부 `.next/`(거름 통과 0 · 유휴 0) · 전량 해시 16k 파일 1.2초.
...
| 성능 | 조건부 이수 | 🟠 고정 2초 창이 큰 트리에서 열려 ×2 복귀(2,500사건/s 상한 실측) · 🟡 tmp 통과 · `status()` 상한 없음·직렬 · `distDir` 비기본은 술어 밖(종전부터) | 침묵 창(2초 슬라이딩 · 상한 30초) · tmp 술어 · `status` 3초 상한 · 교체 두 문 `Promise.all` · `distDir` 은 이 트랜치 밖(아래) |
