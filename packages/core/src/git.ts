    ...
    const where =
        git.branch !== null && short !== null
            ...
            : short !== null
              ? `(분리됨) @ ${short}`
              : "아직 커밋 없음";
    const changes = git.uncommitted > 0 ? `커밋하지 않은 변경 ${count(git.uncommitted)}개` : "깨끗함";
