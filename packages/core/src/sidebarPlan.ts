...
export function sidebarPlan(state: SidebarState): PlanGroup[] {
    ...
    // ⚠ **「불러오기」에 넣지 않는다.** 그 묶음은 «새 빈 폴더로만 간다»는 약속이고 시험이 그것을
    //    문다 — 지금 폴더를 지우는 명령을 그 옆에 두면 약속이 거짓이 된다.
    //
    //    그렇다고 팔레트에만 두는 것도 답이 아니었다. **만드는 사람이 못 찾으면 고객은 더 못
    //    찾는다**(실측: 오너가 발행 직후 못 찾았다). 반복 작업인데 매번 타이핑하게 두는 것도 비용이다.
    //    그래서 **자기 묶음**을 준다 — 묶음 이름과 설명이 경고를 지고, 확인 창이 한 번 더 막는다.
    //
    //    ⚠ **사이트 유무로 숨기지 않는다.** 묶음이 상황에 따라 나타났다 사라지면 화면이 흔들리고,
    //      없는 것은 「갱신이 안 됐다」로 읽힌다(오너 확정). 못 하는 이유는 **누를 때** 말한다.
    {
        groups.push({
            id: "replace",
            label: "소스 갈아 끼우기",
            icon: "replace-all",
            tooltip: "지금 폴더를 받으신 zip 으로 바꿉니다 — 새 폴더로 가지 않습니다",
            items: [
                act(
                    "zip 으로 갱신",
                    "zalkera.site.updateZip",
                    "replace-all",
                    "대행사가 보낸 새 zip 으로 지금 폴더를 갈아 끼웁니다. 지금 내용은 사라집니다",
                ),
            ],
        });
    }
