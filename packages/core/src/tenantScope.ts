...
export const say = {
    ...
    /**
     * **포장 갭** — 우리가 접은 예측과 서버가 저장한 판 지문이 갈렸다(memo191 ⑵).
     *
     * 🔴 **고객 잘못이 아니다.** 발행은 성공했고 판은 서 있다. 갈린 것은 **우리 도구의 포장 규칙**이고,
     *    그대로 두면 이 폴더는 발행 직후부터 영구히 「다름」으로 보인다. 그러니 문장이 사과가 아니라
     *    **사실과 다음 할 일**을 말해야 한다 — 사람이 고칠 수 있는 것이 없으므로 「확인해 주세요」류를
     *    쓰지 않는다.
     */
    packingGap(tenant: CapturedTenant, revisionNo: number): string {
        return (
            `「${shown(tenant)}」 버전 ${countJosa(revisionNo, "은/는")} 올라갔지만, ` +
            `이 도구가 예상한 판 지문과 서버가 저장한 값이 다릅니다. 도구 쪽 문제입니다 — ` +
            `자세히 눌러 나온 두 값을 알려 주시면 고치겠습니다.`
        );
    },
    /**
