...
export type DevtoolsErrorCode =
     ...
     * 서버가 **동의를 요구했다.**
     *
     * ⚠ **AI 변경이 걸린 자리만이 아니다.** 서버는 편집만 있어도 이 동의를 요구한다
     *   (`discardToCurrent` 의 `(draft != null || pending > 0)`). 그래서 이 코드를 「크레딧이
     *   걸렸다」로 읽으면 안 된다 — 무엇이 걸렸는지는 서버가 `errors[].field` 로 짚어 준다.
     *
    ...
    /**
     * 사람이 목록을 보고 답하는 사이 **사이트 쪽 편집이 달라졌다** — 아무것도 안 버렸다.
     *
     * ⚠ 보여 준 것과 버리는 것이 갈리면 그 한 글자 동의가 **본 적 없는 것**을 태운다. 목록을
     *   보고 답하는 창은 상한이 없어(분 단위일 수 있다) 그 사이 콘솔·AI 레인이 얹을 수 있다.
     */
    | "DRAFT_MOVED_WHILE_CONFIRMING"
    /**
     * 장부 기준 판이 **아직 켜지지 않았다 — 짓는 중이다.**
     *
     * ⚠ 「기준이 움직였다」와 **다른 사실이다.** 발행이 곧 활성 전환인 것은 STATIC 뿐이고,
     *   NEXT_SOURCE 는 빌드가 끝나야 켜진다. 둘을 뭉치면 방금 발행한 사람에게 번호가 **거꾸로**
     *   된 문장이 나가고, 그 안내를 따르면 방금 발행한 내용이 폴더에서 사라진다.
     */
    | "PUSH_BASE_BUILDING"
    /** 장부 기준 판이 **짓다가 실패해** 끝내 안 켜졌다. 라이브는 그 전 판이다. */
    | "PUSH_BASE_BUILD_FAILED"
    /**
     * 편집이 **옛 판 위**라 발행이 막혔다(좌초).
     *
     * ⚠ 서버 문면의 「되돌린 뒤」는 CLI 어휘에서 `rollback` 인데, 그 명령은 **편집이 걸려 있으면
     *   그 자체가 막힌다.** 그대로 내보내면 두 거절이 서로를 가리킨다. 참인 출구는 버리기다.
     */
    | "PUBLISH_BASE_MOVED"
    /**
