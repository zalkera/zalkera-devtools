...
test("차단1 — 100바이트 초과 경로가 잘리지 않는다(GNU 긴 이름)", async () => {
  ...
  // 백엔드 패커가 LONGFILE_GNU 라 실제로 이 형식이 온다. 잘리면 서로 덮어쓴다.
  //
  // ⚠ **경로가 100바이트를 실제로 넘어야 한다.** 종전 픽스처는 정확히 100바이트라
  //   `tar --format=gnu` 가 `L` 헤더를 **하나도 안 냈고**(헤더 타입 전수 확인), 그래서 이 시험은
  //   GNU 긴이름 파서를 한 줄도 안 밟았다. 상용 페이로드에는 `L` 헤더가 230개 온다 — 덮개가
  //   없던 자리다. `/detail` 한 조각을 더해 실제로 `L` 이 나게 한다(경로 107바이트).
  const deep =
    "src/app/(marketing)/products/[category]/[slug]/components/gallery/detail";
  await mkdir(join(src, deep), { recursive: true });
  await writeFile(
    join(src, deep, "ProductDetailGalleryThumbnailA.tsx"),
    "export const A = 1;",
  );
  await writeFile(
    join(src, deep, "ProductDetailGalleryThumbnailB.tsx"),
    "export const B = 2;",
  );
  execFileSync("tar", [
    "--format=gnu",
  ...
  const target = await tempDir("zalkera-long-out-");
  const count = await extractTarGz(
    await readFile(join(src, "out.tgz")),
    target,
  );
  ...
  ok(
    files.includes("ProductDetailGalleryThumbnailA.tsx"),
    "이름이 잘리지 않는다",
  );
  strictEqual(
    await readFile(
      join(target, deep, "ProductDetailGalleryThumbnailB.tsx"),
      "utf8",
    ),
    "export const B = 2;",
  );
...
test("차단1 — pax 형식도 이름이 온전하다", async () => {
  const src = await tempDir("zalkera-pax-");
  const deep =
    "src/components/verylongdirectoryname/anotherlongsegment/andonemore";
  await mkdir(join(src, deep), { recursive: true });
  await writeFile(
    join(src, deep, "ExtremelyLongComponentFileName.module.css"),
    ".a{}",
  );
  execFileSync("tar", [
    "--format=pax",
  ...
  const target = await tempDir("zalkera-pax-out-");
  await extractTarGz(await readFile(join(src, "out.tgz")), target);
  ok(
    existsSync(join(target, deep, "ExtremelyLongComponentFileName.module.css")),
  );
});
...
test("차단1 — 다룰 수 없는 항목은 조용히 넘어가지 않고 끊는다", async () => {
  ...
  const target = await tempDir("zalkera-type-");
  await rejects(
    () =>
      extractTarGz(
    ...
    (e: unknown) =>
      e instanceof DevtoolsError && /다룰 수 없는/.test(e.message),
  );
});

test("차단2 — 반쯤 남은 node_modules 는 '준비됨'이 아니다", async () => {
  const dir = await tempDir("zalkera-half-");
  ...
  await rejects(
    ...
    (e: unknown) =>
      e instanceof DevtoolsError && e.code === "DEPENDENCIES_FAILED",
  );
...
test("차단3 — git 레포에 .gitignore 가 없으면 만들어 자격증명을 막는다", async () => {
  ...
  const plain = await tempDir("zalkera-plain-");
...
test("차단4 — 큰 파일을 조용히 빼지 않고 이름을 대고 끊는다", async () => {
  ...
  await rejects(
    ...
    (e: unknown) =>
      e instanceof DevtoolsError &&
      e.code === "PACK_FAILED" &&
      /hero\.mp4/.test(e.message),
  );
});

test("자격증명 파일은 형제 이름까지 전부 빠진다", async () => {
  const dir = await tempDir("zalkera-secrets-");
  await writeFile(join(dir, "package.json"), "{}");
  for (const name of [
    ".env",
    ".env.local",
  ...
  ok(
    !packed.buffer.includes(Buffer.from("oqsk_secret")),
    "아카이브 어디에도 키가 없어야 한다",
  );
});
...
test("경고 — .env.local 중복 선언이 남으면 폐기된 키가 이긴다", async () => {
  ...
  const keyLines = merged
    .split("\n")
    .filter((l) => l.startsWith("ZALKERA_STOREFRONT_KEY="));
  strictEqual(
    keyLines.length,
    1,
    `중복이 남으면 dotenv 가 마지막 것을 채택해 영구 401 이 된다: ${keyLines}`,
  );
...
test("경고 — 기존 .env.local 권한을 600 으로 조인다", async () => {
  ...
  const dir = await tempDir("zalkera-perm-");
...
test("경고 — 압축 폭탄은 사람 말로 끊는다", async () => {
  ...
  const bomb = await createZip([
    { path: "bomb.bin", data: Buffer.alloc(210 * 1024 * 1024) },
  ]);
  const target = await tempDir("zalkera-bomb-");
  await rejects(
    () => extractZip(bomb, target),
    (e: unknown) =>
      e instanceof DevtoolsError && /너무 크거나 손상/.test(e.message),
  );
});
...
test("경고 — 항목 65535 초과는 raw RangeError 가 아니라 사람 말", async () => {
  ...
  await rejects(
    () => createZip(entries),
    (e: unknown) =>
      e instanceof DevtoolsError && /파일이 너무 많습니다/.test(e.message),
  );
});
...
test("경고 — 손상된 zip 목록은 raw RangeError 가 아니라 사람 말", async () => {
  ...
  const target = await tempDir("zalkera-broken-");
  await rejects(
...
test("경고 — 프록시 자격증명은 리포트에 찍히지 않는다", async () => {
  ...
  try {
    ...
    ok(
      proxy && !proxy.detail.includes("s3cret"),
      `비밀번호가 새면 안 된다: ${proxy?.detail}`,
    );
    ok(
      proxy!.detail.includes("proxy.corp:8080"),
      "호스트는 보여야 쓸모가 있다",
    );
  } finally {
...
test("경고 — 핸드셰이크 응답이 우리 형식이 아니면 사람 말로 끊는다", async () => {
  ...
  await rejects(
    ...
    (e: unknown) =>
      e instanceof DevtoolsError && /이해하지 못했습니다/.test(e.message),
  );

  const empty = (async () =>
    Response.json({ status: 200, data: null })) as unknown as typeof fetch;
  await rejects(
    () => fetchHandshake("https://api.zalkera.com", "0.1.0", empty),
    (e: unknown) =>
      e instanceof DevtoolsError && /필요한 정보가 없습니다/.test(e.message),
  );
});
...
test("무결성 — 받은 소스가 원장과 다르면 풀지 않는다(B2)", async () => {
  ...
  const dir = await tempDir("zalkera-integrity-");
  const payload = Buffer.from(
    "이건 진짜 tar 가 아니어도 된다 — 해시에서 먼저 걸린다",
  );
  ...
  await rejects(
    ...
    (e: unknown) =>
      e instanceof DevtoolsError && /원본과 다릅니다/.test(e.message),
  );
...
test("무결성 — 서버가 해시를 안 주면 시작 소스는 진행하지 않는다(fail-open 제거)", async () => {
  ...
  await rejects(
    ...
    (e: unknown) =>
      e instanceof DevtoolsError && /검증할 수 없습니다/.test(e.message),
  );
});
