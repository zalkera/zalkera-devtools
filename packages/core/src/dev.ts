...
import { devEngineArgs } from "./nodeOnPath.ts";
...
export interface DevServerOptions {
   ...
   * 정리(발급한 미리보기 키 폐기)도 안 돌고, 세션이 서고 브라우저 탭이 열린다.
   */
  signal?: AbortSignal;
}
...
export async function startDevServer(
  ...
  // ⚠ **Turbopack 은 자기 몫의 `node` 를 PATH 에서 따로 찾는다.** 우리가 넘긴 Node 경로는 이
  //    프로세스에만 적용되고, CSS(PostCSS) 경로에서 네이티브 쪽이 띄우는 손자 프로세스는 그것을
  //    모른다. Node 를 안 깐 컴퓨터에서는 첫 화면이 500 으로 뜬다 — 판정은 `nodeOnPath.ts` 에 있다.
  const engineArgs = devEngineArgs();
  if (engineArgs.length > 0) {
    log(
      "미리보기를 띄웁니다. 이 컴퓨터에 Node.js 가 있으면 더 빠른 엔진을 씁니다 — 없어도 그대로 됩니다.",
    );
  }
  const child = spawn(
    options.nodePath,
    [nextBin, "dev", "--port", String(port), ...engineArgs],
    {
  ...
  const handle = (chunk: Buffer) => {
    for (const line of chunk.toString().split("\n")) {
      ...
      if (/\bReady in\b|started server on|Local:\s+http/i.test(trimmed))
        markReady();
    }
  };
 ...
 * "안 떴습니다"를 보는데 서버는 뒤에서 계속 떠서 **미리보기 키를 든 채 UI 밖에 남았다.**
...
export function translateLog(line: string): string {
  if (/EADDRINUSE/.test(line))
    return `⚠ 그 포트는 다른 프로그램이 쓰고 있습니다. 다른 포트로 다시 켜 주세요.\n${line}`;
  if (/Cannot find module/.test(line))
    return `⚠ 필요한 파일이 빠졌습니다. 의존성 준비를 다시 해 주세요.\n${line}`;
  ...
  if (
    (/ZalkeraError/.test(line) &&
      /시크릿 키|secretKey|ZALKERA_STOREFRONT_KEY|테넌트의 키/.test(line)) ||
    (/\b(401|403)\b/.test(line) && /storefront|zalkera/i.test(line))
  ) {
    return (
      `⚠ 데이터 접속이 거절되었습니다 — 미리보기 자격증명이 해제됐거나 만료됐습니다.\n` +
      `   자격증명은 **한 번에 하나**라, 다른 기계(또는 다른 창)에서 미리보기를 켜면 이쪽이 끊깁니다.\n` +
      `   미리보기를 다시 켜 주세요.\n${line}`
    );
  }
  return line;
