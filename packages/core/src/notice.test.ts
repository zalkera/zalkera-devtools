...
/**
 * ⚠ **방향 재정렬은 표시 위조다.** 폴더 이름은 남이 정할 수 있고(대행사 zip·git clone), 그
 *   이름이 발행 확인 모달의 첫 줄과 사이드바 머리에 오른다. 강한 재정렬(U+202A~202E)·격리
 *   (U+2066~2069)를 남기면 그 한 줄이 **눈에 보이는 것과 다른 것**이 된다 — 이 소독기가 스스로
 *   「제어문자를 없앤다」고 적어 둔 계약이 그 자리에서 깨져 있었다(보안 심의).
 */
test("방향 재정렬·격리 제어문자를 지운다 — 약한 방향표만 지우던 것", () => {
  for (const ch of ["‪", "‫", "‬", "‭", "‮", "⁦", "⁧", "⁨", "⁩"]) {
    const out = plainNotice(`/srv/${ch}site`, 512);
    ok(!out.includes(ch), `제어문자가 살아남았다: U+${ch.codePointAt(0)?.toString(16)}`);
  }
});
