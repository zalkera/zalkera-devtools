...
const NEEDS: Readonly<Record<string, ReadonlyArray<Need>>> = {
  ...
  "zalkera.site.updateZip": ["signedIn", "site", "siteMatches"],
  ...
  for (const need of NEEDS[command] ?? []) {
    ...
    if (need === "siteMatches" && mismatched(ready)) {
      return {
        ...
        action: {
          label: "이 폴더의 사이트로 돌아가기",
          command: "zalkera.site.useFolder",
        },
        alternative: {
          label: "그 사이트 소스 받기",
          command: "zalkera.site.open",
        },
      };
    }
