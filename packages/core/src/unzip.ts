...
function decodeEntryName(raw: Buffer, flags: number): string {
    ...
    if ((flags & 0x800) === 0) {
        throw new DevtoolsError(
            ...
            // ⚠ **사용자 파일에 대해 단정하지 않는다.** 실제 조건은 「UTF-8 저장 **+ 표시**」라,
            //    UTF-8 로 저장했는데 표시만 없는 zip 도 여기로 온다(맥 기본 압축이 그 형상으로
            //    알려져 있다). 그 사람에게 「UTF-8 이 아니다」는 거짓이고, 「UTF-8 로 저장하는
            //    도구를 쓰라」는 **이미 그렇게 한 도구를 다시 쓰라**는 말이 된다 — 재시도 고리다.
            //    우리가 **안 한 일**을 말하고, 확실히 통하는 길을 먼저 준다.
            "이름의 인코딩 표시가 없어 추측해서 풀지 않았습니다. " +
                "파일 이름을 영문으로 바꾸시면 확실히 됩니다 — 다른 압축 도구로 다시 압축해도 됩니다.",
        );
    }
    try {
