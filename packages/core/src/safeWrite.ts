...
/**
 * **서버가 준 이름으로 파일 이름을 짓기 전에 통과시킨다.**
 *
 * ■ 왜 있나
 *   「예제 zip 다운로드」·「소스 zip 다운로드」는 저장 대화상자의 **기본 경로**를 서버 값
 *   (팩 코드·판 번호·사이트 코드)으로 만든다. 그 값이 `../../` 나 절대 경로를 품으면
 *   `join(base, name)` 이 **base 밖을 가리킨다** — 사람이 대화상자에서 「저장」만 누르면
 *   엉뚱한 자리의 파일이 덮인다. 대화상자가 있다는 것은 방어가 아니다: 기본값이 이미
 *   틀린 곳을 가리키고 있으면 사람은 그것을 읽지 않는다.
 *
 * ■ 무엇을 남기나
 *   영숫자·`.`·`_`·`-` 만 남기고 나머지는 `-` 로 바꾼다. 경로 구분자(`/`·`\\`)와 `..` 가
 *   그 그물에 함께 걸린다. 앞뒤 `.` 는 떼어 숨김 파일·상대 경로 조각이 되지 않게 한다.
 *
 * ■ 비면 부른 쪽이 정한다
 *   전부 걸러져 빈 문자열이 되면 `fallback` 을 준다 — 빈 이름으로 경로를 만들면 폴더를
 *   가리키게 되고, 그 실수는 저장 대화상자에서 눈에 안 띈다.
 */
